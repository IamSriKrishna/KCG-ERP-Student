    ...
    return PopScope(
      ...
      onPopInvoked: (_) async {
        await showCupertinoModalPopup<void>(
          ...
          builder: (context) => CupertinoAlertDialog(
            title: Text(S.current.warning, style: GoogleFonts.merriweather()),
      ...
      child: Scaffold(
        ...
        body: ListView(
          ...
          children: [
            LeaveCustomField(
                label: S.current.name, hint: student.name.toUpperCase()),
            LeaveCustomField(
                label: S.current.rollnumber,
                hint: student.rollno.toUpperCase()),
            LeaveCustomField(
                label: S.current.department,
                hint: student.department.toUpperCase()),
            LeaveCustomField(
            ...
            LeaveCustomField(
                label: days == 1 ? S.current.noofday : S.current.noofdays,
                hint: days == 1
                    ? '${days} ${S.current.day}'
                    : '${days} ${S.current.days}'),
            LeaveCustomFieldTwo(
              label: S.current.reason,
              hint: S.current.enteryourreason,
              reason: _reason,
            ),
            Padding(
              padding:
